 ...
 * @returns {any}
 */
export function derive_key(password, existing_salt) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
 ...
 * @returns {string}
 */
export function encrypt_private_key(keypair, derived_key) {
    let deferred3_0;
    let deferred3_1;
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
 ...
 * @returns {Uint8Array}
 */
export function decrypt_private_key(encrypted_key, derived_key) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('mata_auth_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
