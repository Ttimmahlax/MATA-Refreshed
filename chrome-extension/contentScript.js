        ...
        chrome.runtime.sendMessage({ 
          ...
          if (chrome.runtime.lastError) {
            ...
            if (isContextInvalidated) {
              ...
              setTimeout(syncCriticalFiles, 500); // Reduced from 1000ms
            } else {
...
function setupListeners() {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    console.log('[MATA Extension] Content script received message:', message.action || message.type);
    
    try {
      // Handle different message types from background script or popup
      
      // Special case for TRIGGER_INDEXEDDB_BACKUP type sent directly from popup.js
      if (message.type === 'TRIGGER_INDEXEDDB_BACKUP') {
        try {
          console.log(`[MATA Extension] Received request to backup IndexedDB for user: ${message.user}`);
          
          // Only proceed if we have a user
          if (!message.user) {
            sendResponse({ 
              success: false, 
              error: 'No user email specified for backup'
            });
            return true;
          }
          ...
          // Start the backup process for this specific user
          console.log(`[MATA Extension] Starting targeted IndexedDB backup for user ${userEmail}`);
          
          // Create a backup queue with just this user
          const backupQueue = [userEmail];
          
          // Create a Promise to track backup completion
          const backupPromise = new Promise((resolve) => {
            // Custom callback for when backup is complete
            const onBackupComplete = () => {
              console.log(`[MATA Extension] Backup complete for user ${userEmail}`);
              resolve(true);
            };
            
            // Use the existing backupUserDatabases function with our callback
            backupUserDatabases(userEmail, onBackupComplete);
          });
          
          // Wait for backup to complete then send response
          backupPromise.then(() => {
            sendResponse({ 
              success: true, 
              message: `IndexedDB backup completed for user ${userEmail}`,
              timestamp: Date.now()
            });
          }).catch(err => {
            console.error(`[MATA Extension] Error in IndexedDB backup:`, err);
            sendResponse({ 
              success: false, 
          ...
          console.error('[MATA Extension] Error handling backup request:', backupError);
          sendResponse({ 
            success: false, 
      ...
      // Handle standard action-based messages
  ...
  window.addEventListener(MATA_APP_EVENT, function(event) {
    ...
    try {
      ...
      // CLEAR_ACTIVE_USER - Handler for explicit logout cleanup
      if (detail.type === 'CLEAR_ACTIVE_USER') {
        console.log('[MATA Extension] Received CLEAR_ACTIVE_USER request, removing active user data');
        try {
          // Clear active user from localStorage
          localStorage.removeItem('mata_active_user');
          
          // Clear from window cache
          if (window._mataActiveUser) {
            delete window._mataActiveUser;
          }
          
          // Clear any in-memory caches for active user data
          window._mataLocalKeys = {};
          window._mataLocalSalts = {};
          
          // Also clear from chrome.storage for consistency
          chrome.runtime.sendMessage({
            type: 'REMOVE_LOCAL_STORAGE_VALUE',
            key: 'mata_active_user'
          }, response => {
            console.log('[MATA Extension] Active user cleared from chrome.storage:', response);
            
            // Respond to web app
            window.dispatchEvent(new CustomEvent(EXTENSION_RESPONSE_EVENT, {
              detail: {
                type: 'CLEAR_ACTIVE_USER_RESPONSE',
                requestId: requestId,
                success: true,
                message: 'Active user cleared successfully',
                timestamp: Date.now()
              }
            }));
          });
          ...
          console.error('[MATA Extension] Error clearing active user:', error);
          
          // Still respond to web app even if there was an error
          window.dispatchEvent(new CustomEvent(EXTENSION_RESPONSE_EVENT, {
            detail: {
              type: 'CLEAR_ACTIVE_USER_RESPONSE',
              requestId: requestId,
              success: false,
              error: error.message,
              message: 'Failed to clear active user',
              timestamp: Date.now()
            }
          }));
...
function syncCriticalFiles() {
  try {
    ...
    if (activeUser) {
      ...
      // No active user found - this might be after logout
      console.log('[MATA Extension] No active user found - possible logout state');
      
      // Clear the active user from window cache
      if (window._mataActiveUser) {
        delete window._mataActiveUser;
        console.log('[MATA Extension] Cleared stale _mataActiveUser from window cache');
      }
      
      // Clear from chrome.storage to ensure consistency across storage systems
      chrome.runtime.sendMessage({
        type: 'REMOVE_LOCAL_STORAGE_VALUE',
        key: 'mata_active_user'
      }, response => {
        console.log('[MATA Extension] Removed mata_active_user from chrome.storage during sync');
      });
      
      // Reset any in-memory caches to prevent stale data
      window._mataLocalKeys = {};
      window._mataLocalSalts = {};
    }
...
// Listen for user login events to sync data when users log in with a new account
window.addEventListener('user_login', (event) => {
  console.log('[MATA Extension] User login detected, syncing critical files...');
  // Get the event details if available
  const detail = event instanceof CustomEvent ? event.detail : null;
  const userEmail = detail?.email || null;
  
  if (userEmail) {
    console.log(`[MATA Extension] Login detected for user: ${userEmail}, triggering full sync`);
  }
  
  // Perform immediate sync of critical files
  syncCriticalFiles();
  
  // Schedule a second sync after a short delay to ensure all localStorage changes are captured
...
setInterval(syncCriticalFiles, 5000); // Every 5 seconds (reduced from 30s)
